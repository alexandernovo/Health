...
const NewbornDeliveryRecords: React.FC = () => {
    ...
    const handleInputChangePost = (e: React.ChangeEvent<HTMLInputElement> | React.ChangeEvent<HTMLSelectElement | HTMLTextAreaElement>) => {
        const { name, value } = e.target;
                ...
                <div className="card-body p-0">
                    ...
                    <div className='flex px-[24px] gap-5'>
                        <div className='flex flex-col w-1/2'>
                            ...
                            <input type="text" placeholder="Infant's Name" value={newborn?.infantsName} onChange={handleInputChange} className="input input-bordered" />
                        </div>
                        <div className='flex flex-col w-1/2'>
                            <label className='font-semibold text-[14px]'>Date and Time of Delivery</label>
                            <input type="datetime-local" value={newborn?.dateTimeDelivery} onChange={handleInputChange} placeholder="Infant's Name" className="input input-bordered" />
                        </div>
                    </div>
                    <div className='flex px-[24px] gap-5'>
                        ...
                        <div className='flex flex-col w-[33%]'>
                            ...
                            <input type="text" placeholder="Length at Birth" value={newborn?.lengthAtBirth} onChange={handleInputChange} className="input input-bordered" />
                        </div>
                        <div className='flex flex-col w-[33%]'>
                            <label className='font-semibold text-[14px]'>Weight</label>
                            <input type="text" placeholder="Weigth" value={newborn?.infantWeight} onChange={handleInputChange} className="input input-bordered" />
                        </div>
                    </div>
                    <div className='flex px-[24px] gap-5'>
                        <div className='flex flex-col w-1/2'>
                            <label className='font-semibold text-[14px]'>Newborn Screening Code</label>
                            <input type="text" placeholder="Newborn Screening Code" value={newborn?.newBornScreeningCode} onChange={handleInputChange} className="input input-bordered" />
                        </div>
                        <div className='flex flex-col w-1/2'>
                            <label className='font-semibold text-[14px]'>Date of Newborn Screening</label>
                            <input type="date" placeholder="Date of Newborn Screening" value={newborn?.dateOfNewBornScreening} onChange={handleInputChange} className="input input-bordered" />
                        </div>
                    </div>
                    <div className='flex px-[24px] gap-5'>
                        <div className='flex flex-col w-1/2'>
                            <label className='font-semibold text-[14px]'>Date and Time of Discharge</label>
                            <input type="datetime-local" placeholder="Date and Time of Discharge" value={newborn?.dateAndTimeOfDischarge} onChange={handleInputChange} className="input input-bordered" />
                        </div>
                    </div>
                    <div className='flex px-[24px] gap-5'>
                        <div className='flex flex-col w-1/2'>
                            <label className='font-semibold text-[14px]'>APGAR Score</label>
                            <input type="text" placeholder="APGAR Score" value={newborn?.APGARScore} onChange={handleInputChange} className="input input-bordered" />
                        </div>
                        <div className='flex flex-col w-1/2'>
                            <label className='font-semibold text-[14px]'>Presentation</label>
                            <input type="date" placeholder="Presentation" value={newborn?.Presentation} onChange={handleInputChange} className="input input-bordered" />
                        </div>
                    </div>
                        ...
                        <table className="table border mt-1 mb-3">
                            ...
                            <tbody>
                                {postPartrum
                                    ...
                                    .map(pP => (
                                        <tr>
                                            <td className='border'>{pP.postPartrumDate}</td>
                        ...
                        <div className='border p-4 mb-3 rounded-md shadow-sm'>
                            <div className='flex gap-5'>
                                <div className='flex flex-col w-[33%]'>
                                    ...
                                    <input type="date" className="input input-bordered" name='postPartrumDate' value={postPartrumAdd.postPartrumDate} onChange={handleInputChangePost} />
                                </div>
                                <div className='flex flex-col w-[33%]'>
                                    <label className='font-semibold text-[14px]'>Body Temperature</label>
                                    <input type="number" placeholder="Body Temperature" name='bodyTemperature' value={postPartrumAdd.bodyTemperature} onChange={handleInputChangePost} className="input input-bordered" />
                                </div>
                                <div className='flex flex-col w-[33%]'>
                                    <label className='font-semibold text-[14px]'>BP</label>
                                    <input type="text" placeholder="BP" name='postPartrumBP' value={postPartrumAdd.postPartrumBP} onChange={handleInputChangePost} className="input input-bordered" />
                                </div>
                            </div>
                            <div className='flex gap-5 mt-3'>
                                <div className='flex flex-col w-[33%]'>
                                    <label className='font-semibold text-[14px]'>Fundus</label>
                                    <input type="date" placeholder='Fundus' name='postPartrumFundus' value={postPartrumAdd.postPartrumFundus} onChange={handleInputChangePost} className="input input-bordered" />
                                </div>
                                <div className='flex flex-col w-[33%]'>
                                    <label className='font-semibold text-[14px]'>Breast</label>
                                    <input type="text" placeholder="Breast" name='breast' value={postPartrumAdd.breast} onChange={handleInputChangePost} className="input input-bordered" />
                                </div>
                                <div className='flex flex-col w-[33%]'>
                                    <label className='font-semibold text-[14px]'>Lochia</label>
                                    <input type="text" placeholder="Lochia" name='Lochia' value={postPartrumAdd.Lochia} onChange={handleInputChangePost} className="input input-bordered" />
                                </div>
                            </div>
                        ...
                        <div className='ml-4 text-[13px] mt-3'>
                            ...
                            <div className='flex gap-5 mt-2 items-center mb-4'>
                                ...
                                <div className='flex w-[60%] items-center gap-2'>
                                    ...
                                    <input type="text" placeholder="If Yes, please specify:" required={newborn?.discharge === 'YES'} disabled={newborn?.discharge === 'NO' || newborn?.discharge === '' || newborn?.discharge === null || newborn?.discharge === undefined} value={newborn?.specify} onChange={handleInputChange} className="input input-bordered input-sm flex-1" />
                                </div>
                            </div>
                            <div className='flex justify-end mb-[30px] mt-4'>
                                <button className="btn btn-sm btn-primary mt-2 text-white">Save Newborn Delivery Record</button>
                            </div>
                        </div>
                    </div>
                </div>
