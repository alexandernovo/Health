...
const ImmunizationRecord: React.FC = () => {
    ...
    const [immunization, setImmunization] = useState<AppointmentModel[]>([]);
    ...
    const filteredPlanning = immunization.filter((immunization) =>
        DateToString(immunization.appointmentDate)?.toLowerCase().includes(searchQuery.toLowerCase())
    );
    ...
    const fetchImmunizationRecord = async () => {
        setLoading(true);
        try {
            const response = await axios.get(`/api/immunization/getUserImmunizationRecord/${user_id}`, {
                headers: {
                    Authorization: `Bearer ${token}`
                }
            })
            if (response.data.status == "success") {
                console.log(response.data.immunization);
    ...
    const columns: TableColumn<AppointmentModel>[] = [
        ...
        },
        {
            ...
            cell: (row: AppointmentModel) => (
                <Link to={`/immunization_report/${row.appointment_id}`} className={`btn btn-outline btn-xs flex gap-1 btn-primary px-4`}>
        ...
        },
        {
            ...
            width: '13%',
            cell: (row: AppointmentModel) => (
                <Link to={`/immunization_update/${row.appointment_id}`} className={`btn btn-outline btn-xs flex gap-1 btn-primary px-4`}>
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4">
                ...
                <ul>
                    ...
                    <li>
                        <span className="inline-flex gap-2 items-center">
                            ...
                            {user?.firstname} ${getMiddleInitial(user?.middlename || '')} {user?.lastname} {user?.extension} Immunization Records
                        </span>
                    </li>
                </ul>
            </div>
                ...
                <div className="card-body p-0">
                    <div className='flex justify-between p-4 px-5'>
                        <h1 className='flex text-[20px] items-center gap-1 font-semibold'>
                            ...
                            {user?.firstname} ${getMiddleInitial(user?.middlename || '')} {user?.lastname} {user?.extension} Immunization Record
                        </h1>
