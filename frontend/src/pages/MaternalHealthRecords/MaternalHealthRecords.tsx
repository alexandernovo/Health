...
const MaternalHealthRecords: React.FC = () => {
    const { user_id } = useParams<{ user_id: string }>();
    const token: string | null = localStorage.getItem("token");
    const [maternal, setMaternal] = useState<AppointmentModel[]>([]);
    ...
    const filteredMaternal = maternal.filter((maternal) =>
        DateToString(maternal.appointmentDate)?.toLowerCase().includes(searchQuery.toLowerCase())
    );
    ...
    const fetchMaternalRecordsofUser = async () => {
        setLoading(true);
        try {
            const response = await axios.get(`/api/maternal/getUserMaternalRecord/${user_id}`, {
                headers: {
                    Authorization: `Bearer ${token}`
                }
            })
                ...
                <ul>
                    ...
                    <li>
                        <span className="inline-flex gap-2 items-center">
                            ...
                            {user?.firstname} {getMiddleInitial(user?.middlename || '')} {user?.lastname} {user?.extension} Maternal Health Records
                        </span>
                    </li>
                </ul>
            </div>
                ...
                <div className="card-body p-0">
                    <div className='flex justify-between p-4 px-5'>
                        <h1 className='flex text-[20px] items-center gap-1 font-semibold'>
                            ...
                            {user?.firstname} {getMiddleInitial(user?.middlename || '')} {user?.lastname} {user?.extension} Maternal Health Record
                        </h1>
